...
const N8N_WEBHOOK_URL = process.env.NEXT_PUBLIC_N8N_WEBHOOK_RESUME_SEQUENCE || 'https://auto.lincolnwaste.co/webhook/[id]'
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Call n8n webhook to resume sequence
    ...
    if (!n8nResponse.ok) {
      return NextResponse.json(
        { error: n8nData?.error || n8nData?.message || 'Failed to resume sequence' },
